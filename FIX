...
def baca_menu_dari_csv(username): 
    try:
        ...
        menu = { row["Nama Menu"]: row["Harga"] for _, row in stand.iterrows()} 
        return menu 
...
def baca_data_orderan():
    try:
        return pd.read_csv("orderan.csv", header=None, names=["Stand", "Nama Menu", "Jumlah", "Harga", "Status"])
    except FileNotFoundError:
...
def ubah_status_pesanan(stand):
    ...
    print(f"\nPesanan belum diproses :")
    print(tabulate(df_stand, headers=["Nama stand", "Pesanan", "Jumlah", "Status"], tablefmt="fancy_grid"))
...
def baca_voucher_dari_csv():
    try:
        voucher_df = pd.read_csv("voucher.csv")
        voucher_df.columns = ["Kode Voucher", "Diskon"]
        return voucher_df
    except FileNotFoundError:
...
def buat_pesanan(menu_df):
    ...
    if keranjang:
        ...
        if simpan == "y":
            for nama_menu, data in keranjang.items():
                stand_order = menu_df[menu_df["Nama Menu"].str.lower() == nama_menu.lower()]["Stand"]
        ...
        if homepage == 1:
            ...
            match pilihan:
                ...
                case '0':
                    ...
                    break
                case _ :
                    print('\n| Maaf, silahkan masukkan pilihan yang tersedia.')
...
def login():
    ...
    while (i<3):
        ...
        if username1 in daftar_pengguna["username"].values:
            i = 3
            while (j <3):
                password = input("\nMasukkan Password : ")
                user_data = daftar_pengguna[daftar_pengguna["username"] == username1]
...
def registrasi():
    ...
    while(True):
        username2 = input("\nMasukkan username : ")
        if username2 in daftar_pengguna["username"].values:
            print('Maaf, username sudah ada!!!\nSilahkan masukkan username lain')
        else:
            password = input("\nMasukkan Password : ")
            print("\nAnda bisa memilih peran yang akan dijalankan\n[1]Pembeli\n[2]Penjual")
            while(True):
                ...
                if role == '1' :
                    ...
                    print('Pilihan tidak valid. Tolong masukkan opsi 1 atau 2.')
            data_baru = {'username' : username2, 'password' : password, 'role' : role}
            data_baru = pd.DataFrame([data_baru])
            daftar_pengguna = pd.concat([daftar_pengguna, data_baru], ignore_index=True)
            daftar_pengguna.to_csv('DaftarPengguna.csv', index=False)
